...
var pto     = require('./promise_timeout');
var delay	= require('delay');

var description = 'swagger2iodocs';
...
function swagger2iodocs(options) {
	/*******************************
	...
	var promise = new Promise((resolve, reject) => {
		...
		var genRespSample = options.gen_resp ? true : false;
		var validateSwagger = options.validate_swagger ? true : false;
		var replaceAccented = options.replace_acc ? true : false;
		...
		// Load Swagger from online content
		...
		if (swaggerDoc) {
			...
			if (!host) {
				reject("Invalid Swagger document, missing host information");
			}
			...
			apiClient.methods.fetchAllServices(svcArgs, function (serviceList, serviceRawResponse) {
				...
				if (serviceList.length === 0) {
					// not found
					reject("API definition '" + apiName + "' not found'");
				} else {
					if (serviceList.length === 1) {
						...
						if (serviceList.length > 1 && !printOnly) {
							// more than one match
							reject("Multiple APIs named '" + apiName + "' found");
						}
					}
				}
				if (swaggerDoc.apis) { // Swagger 1.2
					reject("Swagger apis 1.2 not supported.");
				} else if (swaggerDoc.paths) { // Swagger 2.0
					processSwagger20(resolve, reject, swaggerDoc, apiId, apiName, basePath);
				}
			});
		}
		 ...
		 * Process Swagger 2.0 *
		 ***********************/
		var processSwagger20 = function(resolve, reject, swaggerDoc, apiId, apiName, basePath) {
			var epArgs;
			var httpMethods = [];
			var methods = {};
			var ep;
			...
			if (swaggerVer === "2.0" && validateSwagger) {
				swagger.validate(swaggerDoc, function(err, api) {
					if (err && err.message) {
						reject("Swagger 2.0 validation error: " + JSON.stringify(err.message, null, 2));
					}
				});
			}
						...
						delay(1000).then(() => {
							if (ioDocDef.errorCode && ioDocDef.errorCode === 404) {
								...
								apiClient.methods.createIODocs(ioData, function (ioDoc, createRawResponse) {
									...
									renderOutput(resolve, reject, 'create');
								});
							} else {
								...
								apiClient.methods.updateIODocs(ioData, function (ioDoc, updateRawResponse) {
									//console.log(JSON.stringify(ioDoc, null, 3));
									renderOutput(resolve, reject, 'update');
								});
							}
						});
					});
		 ...
		 * Render output *
		 *****************/
		var renderOutput = function(resolve, reject, action) {
